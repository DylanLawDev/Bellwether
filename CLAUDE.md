...
**Currently on `main` (T00–T12 merged):** the full v0 unstructured spine, end to end — `config.py`, `db.py`, migrations + the 6-table schema, the GCS bronze store, the `Submission`/`IngestResult` contracts, `enqueue/lease/ack/fail`, `ingest.py` (bronze-first ingest + dedup), `api.py` (FastAPI front door), `client.py`, `extractors/` (registry + GDELT GKG), `worker.py` + `gold.py`, and the reference `producers/gdelt/`. **Not yet built (T13–T14):** the Terraform `infra/` and the Dockerfile/deploy pipeline. New feature work happens on a **ticket branch**, not directly on `main`.
